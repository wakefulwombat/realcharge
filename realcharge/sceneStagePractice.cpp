...
SceneStagePractice::SceneStagePractice(ISetNextScene* changer, ISetDifficulty* isetDifficulty, int difficultyNum, int stageNum, int modeNum){
	...
	if (modeNum == 0){//explode
		...
		this->enemyManage = new EnemyManage(this->barrageManage, this->itemManage, this->player, this->board, this->bossManage, this->bossManage, this->enemyBulletManage, this->burnEffectManage, this->backEffectManage);
		this->shockWaveManage = new ShockWaveManage(this->enemyManage, this->bossManage, this->bossManage);

		this->scenePause = new ScenePause(changer, isetDifficulty, this, this->backEffectManage);

		this->enemyBulletManage->initialize(this->burnEffectManage, this->shockWaveManage, this->player, this->bossManage, this->bossManage);
		this->player->initialize(this->numericalBarManage, this->itemManage, this->enemyBulletManage, this->bossManage, this->shockWaveManage, this->bombManage, difficultyNum, false);
	}
	else if (modeNum == 1){//stand
		this->itemManage = new ItemManage();
		this->burnEffectManage = new BurnEffectManage(1);
		this->backEffectManage = new BackEffectManage();
		this->numericalBarManage = new NumericalBarManage();
		...
		this->enemyBulletManage->initialize(this->burnEffectManage, this->shockWaveManage, this->player, this->bossManage, this->bossManage);
		this->player->initialize(this->numericalBarManage, this->itemManage, this->enemyBulletManage, this->bossManage, this->playerBulletManage, this->enemyBulletManage, difficultyNum, false);
	}
	else{//absorb
		this->itemManage = new ItemManage();
		this->burnEffectManage = new BurnEffectManage(1);
		...
		this->enemyBulletManage->initialize(this->burnEffectManage, this->shockWaveManage, this->player, this->bossManage, this->bossManage);
		this->player->initialize(this->numericalBarManage, this->itemManage, this->enemyBulletManage, this->bossManage, this->shockWaveManage, this->collapsarManage, difficultyNum, false);
	}
}

void SceneStagePractice::initialize(){
	if (this->modeNum == 0){//explode
	...
	this->loadStage();
	Music::playRoadMusic(this->stageNum);
}

void SceneStagePractice::update(){
	if (this->modeNum == 0){//explode
...
void SceneStagePractice::finalize(){
	if (this->modeNum == 0){
...
void SceneStagePractice::loadEnemyCSV(){
	char filepath[64];

	sprintf_s(filepath, "material\\enemyCSV\\%d\\e%d.csv", this->difficultyNum, this->stageNum);
...
void SceneStagePractice::loadBarrageCSV(){
	char filepath[64];
	sprintf_s(filepath, "material\\barrageCSV\\%d\\b%d.csv", this->difficultyNum, this->stageNum);
	int fp = FileRead_open(filepath);
	while (FileRead_getc(fp) != '\n');
...
void SceneStagePractice::loadBossEventCSV(bool isStartEvent){
	char filepath[64];
	if (isStartEvent == true) sprintf_s(filepath, "material\\boss_talk\\s%d.csv", this->stageNum);
	else sprintf_s(filepath, "material\\boss_talk\\e%d.csv", this->stageNum);
	int fp = FileRead_open(filepath);
	while (FileRead_getc(fp) != '\n');
...
void SceneStagePractice::loadStage(){
	ClearDrawScreen();
...
bool SceneStagePractice::isHitPointSquare(Point p_point, Point sq_center, Size sq_size){
...
bool SceneStagePractice::isHitPointCircle(Point p_point, Point ci_center, double r){
	return ((p_point.x - ci_center.x)*(p_point.x - ci_center.x) + (p_point.y - ci_center.y)*(p_point.y - ci_center.y) < r*r);
}

bool SceneStagePractice::isHitSquareSquare(Point center1, Size size1, Point center2, Size size2){
...
bool SceneStagePractice::isHitCircleCircle(Point center1, double r1, Point center2, double r2){
	return ((center1.x - center2.x)*(center1.x - center2.x) + (center1.y - center2.y)*(center1.y - center2.y) < (r1 + r2)*(r1 + r2));
}

void SceneStagePractice::hitCheckAllObject_Explode(){
	//プレイヤーと敵
...
void SceneStagePractice::hitCheckAllObject_Stand(){
	//プレイヤーと敵
	for (int i = 0; i<this->enemyManage->getEnemyVectorSizeMax(); i++){
		if (this->isHitPointSquare(this->player->getPosition(), this->enemyManage->getEnemyPosition(i)->igetPosition(), this->enemyManage->getEnemySize(i)) == true){
			if (this->player->getCharactorStatus() == ChStatus_Normal) {
...
void SceneStagePractice::hitCheckAllObject_Absorb(){
	//プレイヤーと敵
	for (int i = 0; i < this->enemyManage->getEnemyVectorSizeMax(); i++){
		if (this->isHitPointSquare(this->player->getPosition(), this->enemyManage->getEnemyPosition(i)->igetPosition(), this->enemyManage->getEnemySize(i)) == true){
			if (this->player->getCharactorStatus() == ChStatus_Normal) {
...
void SceneStagePractice::isetNextStage(){
